...
bool Mesh::fromVertices(const std::vector<vec2>& inputVertices,
	...
	vec2& out_size)
{
	...
	// Define a default color for the polygon
	vec3 defaultColor = { 1.0f, 1.0f, 1.0f };

	// Convert input vertices to ColoredVertices
	for (const auto& vert : inputVertices) {
		vec3 position3D = { vert.x, vert.y, 0.0f };
		out_vertices.push_back({ position3D, defaultColor });
	}

	// Generate indices for a fan triangulation (assuming convex polygon)
	for (size_t i = 1; i < inputVertices.size() - 1; ++i) {
		out_vertex_indices.push_back(0);
		out_vertex_indices.push_back(uint16_t(i));
		out_vertex_indices.push_back(uint16_t(i + 1));
	}

	// Compute bounds of the mesh
	vec3 max_position = { -FLT_MAX, -FLT_MAX, -FLT_MAX };
	vec3 min_position = { FLT_MAX, FLT_MAX, FLT_MAX };
	for (const ColoredVertex& pos : out_vertices) {
		max_position = glm::max(max_position, pos.position);
		min_position = glm::min(min_position, pos.position);
	}
	if (abs(max_position.z - min_position.z) < 0.001) {
